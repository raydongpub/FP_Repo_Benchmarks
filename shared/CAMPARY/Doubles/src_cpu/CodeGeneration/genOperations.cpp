...
static void print_fast_renorm2L_inPlace(int const sX, int const sR){
  ...
  renormFct << "inline void fast_renorm2L<" << sX << "," << sR << ">(double *x){\n";
	...
	renormFct << "\tdouble pr = fast_two_sum(x[" << sX-2 << "], x[" << sX-1 << "], x[" << sX-1 << "]);\n";
...
static void print_fast_renorm2L(int const sX, int const sR){
  ...
  renormFct.open("../Specs/specRenorm.h",ios::app);
	...
	renormFct << "template<>\n";
  renormFct << "inline void fast_renorm2L<" << sX << "," << sR << ">(const double *x, double *r){\n";

	renormFct << "\tdouble f[" << sX << "];\n\tint ptr=0;\n";

	renormFct << "\tdouble pr = fast_two_sum(x[" << sX-2 << "], x[" << sX-1 << "], f[" << sX-1 << "]);\n";
	for(int i=sX-3; i>0; i--)	
...
static void print_renorm2L_4Add1(int const sX, int const sR){
  ...
  renormFct.open("../Specs/specRenorm.h",ios::app);
	...
	renormFct << "template<>\n";
  renormFct << "inline void renorm2L_4Add1<" << sX << "," << sR << ">(const double *x, const double y, double *r){\n";

	renormFct << "\tdouble f[" << sX+1 << "];\n\tint ptr=0;\n";

	renormFct << "\tdouble pr = two_sum(x[" << sX-1 << "], y, f[" << sX << "]);\n";
...
static void print_baileyAdd(int const K, int const L, int const R){
  if(L==1){
    ...
    addFct.open("../Specs/specAddition.h",ios::app);
	  ...
	  addFct << "template<>\n";
    addFct << "inline void baileyAdd_fast<" << K << "," << L << "," << R << ">(double const *x, double const *y, double *z){\n";
...
static void print_baileyMul(int const K, int const L, int const R){
  ...
  mulFct.open("../Specs/specMultiplication.h",ios::app);
	...
	mulFct << "template<>\n";
  mulFct << "inline void baileyMul_fast<" << K << "," << L << "," << R << ">(double const *x, double const *y, double *z){\n";
	if(L>1)	mulFct << "\tdouble p, e, f[" << R+1 << "];\n";
	else mulFct << "\tdouble e, f[" << R+1 << "];\n";
...
// campary library variables definition
