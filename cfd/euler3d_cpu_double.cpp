...
{
  {
    std::ofstream file("density_dp.txt");
    file << nel << " " << nelr << std::endl;
    for(int i = 0; i < nel; i++) 
      file << std::fixed << std::setprecision(36) << variables[i*NVAR + VAR_DENSITY] << std::endl;
  }

  {
    std::ofstream file("momentum_dp.txt");
    file << nel << " " << nelr << std::endl;
    for(int i = 0; i < nel; i++)
    {
      for(int j = 0; j != NDIM; j++) 
        file << std::fixed << std::setprecision(36) << variables[i*NVAR + (VAR_MOMENTUM+j)] << "\n";
      file << std::endl;
    }
  }

  {
    std::ofstream file("density_energy_dp.txt");
    file << nel << " " << nelr << std::endl;
    for(int i = 0; i < nel; i++) 
      file << std::fixed << std::setprecision(36) << variables[i*NVAR + VAR_DENSITY_ENERGY] << std::endl;
  }
}
...
void compute_flux(int nelr, int* elements_surrounding_elements, double* normals, double* variables, double* fluxes)
{
  ...
  #pragma omp parallel for default(shared) schedule(static) //reduction(+: nb_count_I, nb_count_II, nb_count_III)
  for(int i = 0; i < nelr; i++)
  {
    int j, nb;
  ...
  // profiling
  // std::cout << "nb_count_I   = " << nb_count_I   << "\n";
  // std::cout << "nb_count_II  = " << nb_count_II  << "\n";
  // std::cout << "nb_count_III = " << nb_count_III << "\n";
}
...
int main(int argc, char** argv)
{
  ...
  // {
  //   std::cout << "ff_var[" << i << "] = ";
  //   std::cout << std::fixed << std::setprecision(80) << ff_variable[i] << std::endl;  
  // }
